...
public class XPathParser {
    ...
    private XPathEvaluator collectXPathEvaluator() {
        if (noEvalAllow) {
            return (XPathEvaluator) new DefaultXPathEvaluator(null, elementOperator);
        }

        if (evals.size() == 1)
            return (XPathEvaluator) new DefaultXPathEvaluator(evals.get(0), elementOperator);

        return (XPathEvaluator) new DefaultXPathEvaluator(new CombiningEvaluator.And(evals), elementOperator);
    }
